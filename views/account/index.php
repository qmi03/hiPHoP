...
<section class="bg-white mx-5 lg:mx-20 p-10 relative top-[-10px]">
  <form action="/account?update=true" method="post" class="flex flex-col lg:grid grid-cols-[200px_1fr] gap-x-5 lg:gap-y-5 text-xl">
    ...
    <input class="px-2 py-1 border-gray-300 border-2 rounded-sm focus:border-pink-200 shadow-gray shadow-sm text-lg italic bg-gray-100" id="username" value="<?= htmlspecialchars($data["username"]) ?>" disabled>
    ...
    <input type="email" class="px-2 py-1 border-gray-300 border-2 rounded-sm focus:border-pink-200 shadow-gray shadow-sm text-lg italic bg-gray-100" value="<?= htmlspecialchars($data["email"]) ?>" disabled>
