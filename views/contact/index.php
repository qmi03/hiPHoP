...
<section class="bg-white mx-5 lg:mx-20 relative top-[-10px] text-lg md:flex items-center justify-between gap-5">
  ...
  <ul class="flex-2/3 px-5 py-5 list-none md:mt-5 text-2xl">
    <li class="flex gap-2 underline text-cyan-600"><span>📧</span><a href="mailto:<?= htmlspecialchars($data["email"]) ?>"><?= htmlspecialchars($data["email"]) ?></a></li>
    <li class="mt-3 flex gap-2"><span>📞</span><a href="tel:<?= htmlspecialchars($data["phone"]) ?>" class="underline text-cyan-600"><?= htmlspecialchars($data["phone"]) ?></a></li>
    <li class="mt-3 flex gap-2"><span>📍</span><a href="#map" class="underline text-cyan-600"><?= htmlspecialchars($data["address"]) ?></a></li>
  </ul>
</section>
...
<section class="mt-10 bg-white mx-5 lg:mx-20 relative">
  ...
  <div id="map" class="flex items-center justify-center">
    <iframe src="https://maps.google.com/maps?q=<?= htmlspecialchars($data["latitude"]) ?>,<?= htmlspecialchars($data["longitude"]) ?>&hl=es;z=14&amp;output=embed" class="w-[100%] min-h-[500px]"></iframe>
  </div>
</section>
