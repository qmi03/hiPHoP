    ...
    <nav id="sidemenu" class="translate-x-full transition-transform transition-duration-[2s] lg:hidden bg-violet-700 fixed top-0 right-0 pl-18 pr-32 pt-28 h-[100vh] z-60">
      ...
      <?php else: ?>
        <div class="m-4 mb-8 text-lg">
          <a class="relative text-white text-xl font-bold items-center flex gap-2 <?= $path == "/account/" ? "" : "opacity-90 hover:opacity-100 hover:italic" ?> cursor-pointer" href="/account">
  ...
  <footer class="m-0 pb-16 bg-violet-900 p-10 px-[3vw] md:px-[5vw] w-[100vw] absolute bottom-0 text-white">
    <div class="flex flex-col md:flex-row gap-5 md:justify-between">
      <div class="flex flex-col md:flex-row gap-3 md:justify-between">
        ...
        <div class="text-lg">
          <p>Contact Us!</p>
          <p><?= htmlspecialchars($data["phone"]) ?> (v)</p>
          <a href="mailto:<?= htmlspecialchars($data["email"]) ?>" class="underline text-cyan-400"><?= htmlspecialchars($data["email"]) ?></a>
        </div>
      </div>
      <div class="flex flex-row gap-5">
        <a href="<?= htmlspecialchars($data["facebook"]) ?>" target="_blank">
        ...
        <a href="<?= htmlspecialchars($data["github"]) ?>" target="_blank">
